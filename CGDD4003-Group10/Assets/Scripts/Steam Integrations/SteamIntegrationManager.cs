...
public class SteamIntegrationManager : MonoBehaviour
{
  ...
  public void addDeath()
  {
    if (connectedToSteam)
    {
      try
      {
        ...
        if (SteamUserStats.GetStatInt("deaths") < 100)
        {
          SteamUserStats.AddStat("deaths", 1);
          SteamUserStats.StoreStats();
          SteamUserStats.IndicateAchievementProgress("oof", SteamUserStats.GetStatInt("deaths"), 100);
        }
      }
      catch (System.Exception e)
      {
  ...
  public void addFruit()
  {
    if (connectedToSteam)
    {
      ...
      {
        SteamUserStats.RequestCurrentStats();
        if (SteamUserStats.GetStatInt("fruits") < 9)
        {
          SteamUserStats.AddStat("fruits", 1);
          SteamUserStats.StoreStats();
          SteamUserStats.IndicateAchievementProgress("nom", SteamUserStats.GetStatInt("fruits"), 9);
        }
      }
      catch (System.Exception e)
      {
  ...
  public void addEnding()
  {
    if (connectedToSteam)
    {
      ...
      {
        SteamUserStats.RequestCurrentStats();
        if (SteamUserStats.GetStatInt("endings") < 3)
        {
          SteamUserStats.AddStat("endings", 1);
          SteamUserStats.StoreStats();
          SteamUserStats.IndicateAchievementProgress("triple_threat", SteamUserStats.GetStatInt("endings"), 3);
        }
      }
      catch (System.Exception e)
      {
  ...
  public void checkCompletion()
  {
    if (connectedToSteam)
    {
      try
      {
        ...
        SteamUserStats.SetStat("achievements", numberUnlocked);
        SteamUserStats.StoreStats();
        SteamUserStats.IndicateAchievementProgress("completed", SteamUserStats.GetStatInt("achievements"), totalAchievements);
      }
      catch (System.Exception e)
      {
        Debug.Log($"Some Steam error occured - {e.Message}");
